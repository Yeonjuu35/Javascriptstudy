window.onload = function(){
    ...
    tabbtn.forEach(function(ele, idx){
        ele.addEventListener('click', function(){
            // this.classList = 'on'; <- = 때문에 값이 초기화 됨
            // this.classList += ' on'; <- 'on'으로 쓰면 값이 계속 추가되기 때문에 onon...식으로 되기 때문에 꼭 앞에 띄어쓰기 넣어주기
            // this.classList.add('on') // <- 이쪽이 훨씬 가볍고 쓰기 편함(동사 처리했기 때문에)
            // 반대는 remove
